...
async function main() {
    ...
    try {
        ...
        for (const type of types) {
            ...
            let typeNumParam = "";
            let typeDetailName = "";
            let currentTypeName = type.Name; // List gives us Original Type Name
            
            for (const p of info.Parameters || []) {
                if (p.Name === "圖說名稱") typeNameParam = p.Value;
                if (p.Name === "詳圖圖號") typeNumParam = p.Value;
                if (p.Name === "詳圖名稱") typeDetailName = p.Value;
                // Double check current type name parameter if available
                if (p.Name === "類型名稱" || p.Name === "Type Name") {
                    currentTypeName = p.Value;
                }
            }
            ...
            if (targetSheetNumber) {
                ...
                if (typeNumParam !== targetSheetNumber) {
                    console.log(`[UPDATE] Type ${type.Id} (${currentTypeName}): Updating 詳圖圖號 from '${typeNumParam}' to '${targetSheetNumber}'`);
                ...
                // 2. Check 類型名稱 (Type Name)
                // New logic: Type name should start with the new sheet number
                // Standard convention is [SheetNumber]-[DetailName]
                const targetTypeName = `${targetSheetNumber}-${typeDetailName}`;
                
                if (currentTypeName !== targetTypeName) {
                    // It doesn't match! Update it!
                    console.log(`[UPDATE] Type ${type.Id} (${currentTypeName}): Renaming Type to '${targetTypeName}'`);
                    try {
                        await callTool('modify_element_parameter', {
                            elementId: type.Id,
                            parameterName: "類型名稱", // Try matching the localized name
                            value: targetTypeName
                        });
                        updatedNames++;
                        needsUpdate = true;
                    } catch (err) {
                        console.log(`   --> Failed to rename via '類型名稱', trying 'Type Name'...`);
                        try {
                            await callTool('modify_element_parameter', {
                                elementId: type.Id,
                            ...
                            console.log(`   --> Failed to rename type parameter! Error:`, err2);
                        }
                    }
                }
