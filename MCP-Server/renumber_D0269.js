...
const renumberMap = [
    // Sequence starts after D0269 (8093973)
    ...
    { id: 10126927, target: "ARB-D0282", name: "一層排水平面圖(3/3)" }, // Wait, D0282? Should be D0285.
    // Correcting my manual list:
...
async function main() {
    ...
    try {
        ...
        for (const item of renumberMap) {
            ...
            console.log(`Renamed Id:${item.id} to ${tempVal}`);
        }
        ...
        for (const item of renumberMap) {
            ...
            console.log(`Renamed Id:${item.id} to ${item.target} (${item.name})`);
        }
