...
const dataFile = 'E:/RevitMCP/scratch_csv/mismatch_v8.json';
const data = JSON.parse(fs.readFileSync(dataFile, 'utf8'));
const updates = data.changed;

console.log(`Starting Round 8 force update of ${updates.length} sheets (Total ${updates.length * 2} operations)...`);
...
function processNext() {
    if (currentIdx >= updates.length) {
        if (phase === 1) {
            ...
            console.log("\n✅ Round 8 synchronization finished!");
            mcp.kill();
            process.exit(0);
        }
        return;
    ...
    if (phase === 1) {
        const tempValue = item.TargetNumber + "_TEMP_R8";
