...
const dataFile = 'E:/RevitMCP/scratch_csv/d07_renumber_map.json';
const data = JSON.parse(fs.readFileSync(dataFile, 'utf8'));
const updates = data.changed;

console.log(`Starting D07 force renumber of ${updates.length} sheets (Total ${updates.length * 2} operations)...`);
...
function processNext() {
    ...
    if (phase === 1) {
        const tempValue = item.Target + "_TMP_RE";
        process.stdout.write(`PHASE 1 [${currentIdx + 1}/${updates.length}]: ${item.Id} (${item.Current}) -> ${tempValue}... `);
        sendCommand(item.Id, tempValue);
    } else {
        process.stdout.write(`PHASE 2 [${currentIdx + 1}/${updates.length}]: ${item.Id} (${item.Current}) -> ${item.Target}... `);
