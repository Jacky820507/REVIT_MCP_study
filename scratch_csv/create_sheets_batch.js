...
// --- Config ---
const CSV_PATH = 'E:/RevitMCP/scratch_csv/sheets_utf8.csv';
const TITLE_BLOCK_ID = 4428;
const BATCH_SIZE = 50; // 每批次建立的圖紙數量
const SKIP_FIRST = 10; // 跳過前 N 張已建立的圖紙
            ...
            if (res.id) {
                if (res.error || (res.result && res.result.isError)) {
                    ...
                    if (res.result && res.result.content) {
                        content = res.result.content.map(c => c.text).join('');
                    }
                    try {
                        const created = JSON.parse(content);
                        console.log(`  ✅ 批次 ${batchIdx + 1} 成功建立 ${created.length} 張圖紙`);
