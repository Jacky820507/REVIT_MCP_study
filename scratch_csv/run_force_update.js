...
const forceData = JSON.parse(fs.readFileSync('E:/RevitMCP/scratch_csv/force_list.json', 'utf8'));
...
function processNext() {
    ...
    if (stage === 1) {
        process.stdout.write(`Stage 1 (TEMP): [${item.Id}] ${item.RevitName} -> ${item.TargetNumber}_TEMP... `);
        sendCommand(item.Id, "Sheet Number", item.TargetNumber + "_TEMP");
    } else {
        process.stdout.write(`Stage 2 (FINAL): [${item.Id}] ${item.RevitName} -> ${item.TargetNumber}... `);
